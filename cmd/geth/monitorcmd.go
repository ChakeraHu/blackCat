...
var (
	...
	monitorCommand = cli.Command{
		...
		Description: `
The Gokc monitor is a tool to collect and visualize various internal metrics
...
func monitor(ctx *cli.Context) error {
	...
	// Attach to an Okcoin node over IPC or RPC
	endpoint := ctx.String(monitorCommandAttachFlag.Name)
	if client, err = dialRPC(endpoint); err != nil {
		utils.Fatalf("Unable to attach to gokc node: %v", err)
	}
	defer client.Close()
	...
	if len(monitored) == 0 {
		...
		if len(list) > 0 {
			...
			utils.Fatalf("No metrics collected by gokc (--%s).\n", utils.MetricsEnabledFlag.Name)
		}
	}
...
// retrieveMetrics contacts the attached gokc node and retrieves the entire set
